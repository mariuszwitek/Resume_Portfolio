...
"\n"
"Usage:\n"
"  maindaq-decoder --file=<codafile> --server=<host> --schema=<db>\n"
"                  --root=<rootfile> [-pgnovh] [--sampling=<factor>] \n"
"                  [--dir=<tmpdir>]\n"
"\n"
"Options:\n"
"  --sampling=<factor>   Decode only 1 in N physics events\n"
"                        [default: 1]\n"
"  --dir=<tmpdir>        Directory for temporary output files \n"
"                        [default: /var/tmp].\n"
"  -p, --prod            Official production. Make an entry in \n"
"                        the summary.production table.\n"
"  -g, --grid            Decode on the Fermi Grid.\n"
"  -n, --nohit           Do not write Hit or TriggerHit output.\n"
"  -o, --online          Decode a CODA file while still being written.\n"
...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_options; i++) {
        ...
        if (help && option->value && !strcmp(option->olong, "--help")) {
            ...
            printf("%s\n", version);
            return 1;
        } else if (!strcmp(option->olong, "--grid")) {
            args->grid = option->value;
        } else if (!strcmp(option->olong, "--help")) {
            args->help = option->value;
        } else if (!strcmp(option->olong, "--nohit")) {
            args->nohit = option->value;
        } else if (!strcmp(option->olong, "--online")) {
            args->online = option->value;
        } else if (!strcmp(option->olong, "--prod")) {
            args->prod = option->value;
        } else if (!strcmp(option->olong, "--verbose")) {
            args->verbose = option->value;
        } else if (!strcmp(option->olong, "--dir")) {
            if (option->argument)
                args->dir = option->argument;
        } else if (!strcmp(option->olong, "--file")) {
            if (option->argument)
                args->file = option->argument;
        } else if (!strcmp(option->olong, "--root")) {
            if (option->argument)
                args->root = option->argument;
        } else if (!strcmp(option->olong, "--sampling")) {
            if (option->argument)
                args->sampling = option->argument;
        } else if (!strcmp(option->olong, "--schema")) {
            if (option->argument)
                args->schema = option->argument;
        } else if (!strcmp(option->olong, "--server")) {
            if (option->argument)
                args->server = option->argument;
        }
    }
    /* commands */
...
DocoptArgs docopt(int argc, char *argv[], bool help, const char *version) {
    DocoptArgs args = {
        0, 0, 0, 0, 0, 0, (char*) "/var/tmp", NULL, NULL, (char*) "1", NULL,
        NULL,
    ...
    Option options[] = {
        ...
        {NULL, "--file", 1, 0, NULL},
        {NULL, "--root", 1, 0, NULL},
        {NULL, "--sampling", 1, 0, NULL},
        {NULL, "--schema", 1, 0, NULL},
        {NULL, "--server", 1, 0, NULL}
    };
