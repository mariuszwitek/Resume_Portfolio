...
public:
    ...
    bool isOriginalInTimePulse() { return (flag & Hit::originalInTimePulse) != 0; }
    ...
    void setOriginalInTimePulse(bool f = true) { f ? (flag |= originalInTimePulse) : (flag &= ~originalInTimePulse); }
    ...
    //Debugging output
    void print() { std::cout << index << " : " << detectorID << " : " << elementID << " : " << pos << " : "
                   << driftDistance << " : " << isInTime() << " : " << isHodoMask() << " : " << isTriggerMask() << std::endl; }
    ...
    enum hitQuality
    {
        ...
        originalInTimePulse = hitFlagBit(4)
    };
    UShort_t flag;
